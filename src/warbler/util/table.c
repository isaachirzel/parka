...
static const usize primeNumbers[] =
{
	11,
	23,
	47,
...
static const usize primeNumberCount = sizeof(primeNumbers) / sizeof(*primeNumbers);

static TableEntry tableEntryCreate(const char *key, usize valueIndex)
{
	assert(key != NULL);
...
static void tableEntryDestroy(TableEntry *entry)
{
	assert(entry != NULL);
...
static void tableEntryDelete(TableEntry *entry)
{
	assert(entry != NULL);
	deallocate(entry->key);
...
static usize getPrimeNumberIndex(usize minCapacity)
{
...
static usize hashKey(const char * restrict key)
{
	assert(key != NULL);
...
static void tableResize(Table *table, usize newPrimeNumberIndex)
{
	assert(table != NULL);
